import styles from "../style";
import { Testimonials } from "../constants";

const Content = ({ text }) => {
  return (
    <div>
      <p className="font-poppins font-normal text-[14px] text-dimWhite my-4 leading-[32px]">
        {text}{" "}
      </p>
    </div>
  );
};

const TestimonialCard = (props) => {
  return(
    <div
      className="flex flex-col px-10 py-12 rounded-[20px] max-w-[370px] md:mr-10 sm:mr-5 mr-0 my-5 feedback-card"
      whileInView={{ y: [-30, 0], opacity: [0, 1] }}
    >
      <div className="flex items-center flex-row gap-[10px] h-[88px]">
          <img
            src={props.logo}
            alt={props.organisation}
            className="w-[52px] h-[52px] rounded-full object-cover"
          />
        <div className="flex flex-col">
          <h4 className="font-poppins font-semibold text-[16px] text-gradient leading-[32px]">
            {props.organisation}
          </h4>
          <p className="font-poppins font-normal text-[14px] text-white my-1 leading-[24px]">
            {props.title}
          </p>
        </div>
      </div>

      <div>
        {props.content.map((info, index) => {
          return <Content key={index} {...info} />;
        })}
      </div>
    </div>
  );
};

const Testimonial = () => {
  return (
    <section id="Testimonial">
      <h1 className="flex-1 font-poppins font-semibold ss:text-[55px] text-[45px] text-white ss:leading-[80px] leading-[80px]">
        Testimonial
      </h1>

      <div className={`${styles.flexCenter} flex-col relative mb-4 mt-8`}>
        <div className="absolute z-[0] w-[60%] h-auto -right-[50%] rounded-full blue__gradient bottom-40" />

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mt-8 md:mt-16 md:grid-cols-3">
          {Testimonials.map((card) => (
            <TestimonialCard key={card.id} index={card.id} {...card} />
          ))}
        </div>
      </div>
    </section>
  );
};

export default Testimonial;
