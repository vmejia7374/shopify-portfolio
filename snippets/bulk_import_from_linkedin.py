...
def convert_volunteering_csv_to_js(input_dir=None, output_file=None):
    ...
    # Generate JS code for Testimonial list
    js_entries = []
...
{",\n".join(content_items)}
    ...
    js_code = "export const Testimonial = [\n" + ",\n".join(js_entries) + "\n];\n"
    ...
    try:
        ...
        if os.path.exists(output_file):
            ...
            # Check if Testimonial is already defined
            if "export const Testimonial" in content:
                # Replace existing Testimonial
                pattern = r"export const Testimonial = \[[\s\S]*?\];"
                content = re.sub(pattern, js_code.strip(), content)
            else:
                # Append Testimonial to the end
                content += "\n\n" + js_code
        else:
            # Create new file with Testimonial
            content = js_code
